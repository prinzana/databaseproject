...
router.put('/update/:Id', async (req, res) => {
    ...
    const { sex, occupation, marital_status, number_of_children, address, state_of_residence, lga_residence, date_of_birth, profile_picture, is_dob_public } = req.body;

    // Validate required fields////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    if (!sex || !occupation || !marital_status || number_of_children === undefined || !address || !state_of_residence || !lga_residence || !date_of_birth) {
        return res.status(400).json({ error: 'Please provide all required fields.' });
    }

    try {
        const sql = `UPDATE additional_details 
                     SET sex = ?, occupation = ?, marital_status = ?, number_of_children = ?, address = ?, 
                         state_of_residence = ?, lga_residence = ?, date_of_birth = ?, profile_picture = ?, 
                         is_dob_public = ?, updated_at = CURRENT_TIMESTAMP 
                     WHERE user_id = ?`;

        const result = await dbRun(sql, [sex, occupation, marital_status, number_of_children, address, state_of_residence, lga_residence, date_of_birth, profile_picture, is_dob_public, userId]);
